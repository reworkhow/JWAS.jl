function MCMC_BayesCC(nIter,mme,df,Pi;
    ...
    BigPi     = zeros(2^nTraits)
    BigPiMean = zeros(2^nTraits)

    #label δs ([0,1,1,0]) as 7 = 6+1 =2^2*1+2^1*1 +1
    #add one to make "0000" label 1
    for key in keys(Pi)
      string4number = ""
      for ichar in key
          string4number = string4number*string(round(Int64,ichar))
      end
      label = parse(Int,string4number,2)+1 #add one to make "00" label 1
      BigPi[label] = Pi[key]
    end
