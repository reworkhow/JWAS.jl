function ssBayesB(matrices::HybridMatrices,
    ...
    for iter = 1:input.chainLength
      ...
      misc.sampleEffectsBayesB!(wGibbs,current,output)
      # sample epsilon
      ...
      for j=1:geno.nMarkers
          current.locusEffectVar[j] = misc.sample_variance(current.α[j],1,input.dfEffectVar, current.scaleVar)
      end
      # sample marker vairance
     ...
     output.genVar[iter] = meanVarg
      ...
      if (iter%outFreq ==0)
       @printf("Iteration %d with %d loci included in the model, mean residual/genetic(imputation) variance %6.3f/%6.3f.\n",
               iter, current.nLoci, meanVare, meanVarg)
      end
    end
