...
const WARMUP_P = parse(Int, get(ENV, "WARMUP_P", "5000"))
const WARMUP_L = parse(Int, get(ENV, "WARMUP_L", "50"))
const OUT_TXT = get(ENV, "BENCH_TXT", "/home/qtlcheng/jwas_full_benchmark_v2_results.txt")
...
function run_case(n::Int, p::Int, l::Int; seed::Int=2026, do_log::Bool=true, cleanup::Bool=true)
    ...
    outdir = "/home/qtlcheng/jwas_full_bench_v2_N$(n)_P$(p)_L$(l)_$(Dates.format(now(), "yyyymmdd_HHMMSS"))"
...
for p in P_LIST
    ...
    block_size = subset[1]["block_size"]
    target_outer = Int(floor(TARGET_L / block_size))
    target_t_mcmc = a + b * target_outer
    ...
    logline(@sprintf("DECOMP P=%d points=%d intercept=%.3f per_outer=%.3f target_outer=%d target_t=%.3f",
                     p, length(subset), a, b, target_outer, target_t_mcmc))
end
...
open(OUT_TXT, "w") do io
    ...
    for c in cases
        @printf(io, "CASE rep=%d N=%d P=%d L=%d outer=%d block_size=%d t_x=%.6f t_geno=%.6f t_mcmc=%.6f seed=%d\n",
                c["rep"], c["N"], c["P"], c["L_input"], c["outer"], c["block_size"], c["t_x"], c["t_geno"], c["t_mcmc"], c["seed"])
    end

    for p in sorted_p
        d = per_p[p]
        @printf(io, "DECOMP P=%d points=%d intercept=%.6f per_outer=%.6f target_outer=%d target_t=%.6f\n",
                p, d["n_points"], d["intercept"], d["per_outer"], d["target_outer"], d["target_t_mcmc"])
    end
