...
- `independent_blocks=false` is the default exact sequential schedule: JWAS refreshes the global corrected phenotype / residual after each block. `independent_blocks=true` instead freezes the sweep-level residual, updates blocks with Julia threads, and merges their deltas after a barrier; this parallel schedule is approximate unless off-block weighted genotype crossproducts (`X_b' W X_c` for `b != c`) are zero.
